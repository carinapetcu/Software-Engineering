...
public class ChairService {
    ...
    public ChairDTO updateChair(ChairDTO dto) {
        var pcMember = pcMemberRepository.findById(dto.getUserID()).orElseThrow(
                () -> new InvalidIDException("Invalid Chair ID: " + dto.getUserID())
        );
        if (pcMember.getAuthority().compareTo(Authority.Chair) == 0) {
            ...
            return converter.entityToDto(pcMemberRepository.save(chair));
        } else {
            throw new InvalidIDException("Invalid Chair ID: " + dto.getUserID());
        }
    }
